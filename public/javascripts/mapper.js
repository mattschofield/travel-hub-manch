...
function trainDepartures(evt, station, app){
  chosenStation = station || $(evt.target).val();
  target = {
    title: "Trains from " + app.stations[chosenStation].title,
    marker: app.stations[chosenStation]
  }
  renderDepartureBoard(target, app);
}

function renderDepartureBoard(target, app){
  $(".item-title").text(target.title);
  $(".departures-board").addClass("active");
  var mkr = target.marker;
  mkr.setAnimation(google.maps.Animation.BOUNCE);
  setTimeout(function(){mkr.setAnimation(null)}, 5000);
  app.map.panTo(mkr.getPosition());
  app.map.setZoom(16);
  app.map.panBy(0, 200);
}
...
function drawTrainStations(app){
  var stations = {MAN: {location: new google.maps.LatLng(53.47732,-2.23121),
                        commonName: "Manchester Piccadilly"
                       },
                  MCV: {location: new google.maps.LatLng(53.48792,	-2.24422),
                        commonName: "Manchester Victoria"
                       },
                  MCO: {location: new google.maps.LatLng(53.47418,	-2.24203),
                        commonName: "Manchester Oxford Road"
                       }
                 };
  for (var stationCode in stations){
    var trainStationMarker = new google.maps.Marker({
        icon: "http://localhost:3000/images/darkredboxed/train.png",
        position: stations[stationCode].location,
        map: app.map,
        title: stations[stationCode].commonName
      })
      app.stations[stationCode] = trainStationMarker;
      google.maps.event.addListener(trainStationMarker, 'click', function(evt){
        trainDepartures(null, stationCode, app);
      });  
  }
...
function draw(app) {
  ...
  if ($('#train-stations-button').hasClass('checked')){
    drawTrainStations(app)
  }
